...
### Dedicated PostgreSQL Management

**`/zerodb-postgres-provision`** - Provision dedicated PostgreSQL instance
- Use for: Creating managed Postgres databases via Railway
...
- Instance states: provisioning, active, maintenance, error

**`/zerodb-postgres-connection`** - Get connection details
- Use for: Direct SQL access via psql, pgAdmin, ORMs
- Returns: Connection string, credentials, host/port
- Credential types: primary, readonly, admin
...
- Filtering: By query type (SELECT, INSERT, UPDATE, DELETE)

**`/zerodb-postgres-restart`** - Restart PostgreSQL instance
- Use for: Applying configuration changes, troubleshooting
- Completion time: 30-60 seconds

**`/zerodb-postgres-delete`** - Delete PostgreSQL instance
- Use for: Removing instances (requires confirmation)
...
### 6. Dedicated PostgreSQL Instance

```
1. /zerodb-postgres-provision - Create Postgres instance (specify size)
2. /zerodb-postgres-status - Wait for provisioning to complete (~2-3 min)
3. /zerodb-postgres-connection - Get connection string and credentials
4. Connect with psql, pgAdmin, or any PostgreSQL client/ORM
5. /zerodb-postgres-usage - Monitor query counts and billing
6. /zerodb-postgres-logs - View query performance and optimize
```
...
## Dedicated PostgreSQL - Example Workflow
...
// 3. Get connection credentials once active
...
/zerodb-postgres-usage
{
  "project_id": "your-project-id",
  "hours": 24
}

// 5. View query logs for optimization
/zerodb-postgres-logs
{
  "project_id": "your-project-id",
  "limit": 50,
  "query_type": "SELECT"
